...
public csdm_PostDeath(killer, victim, headshot, const weapon[])
{
	...
	new players[32], num_players, i, CsTeams:team, stat[CsTeams]
	...
	if (stat[CS_TEAM_CT] - stat[CS_TEAM_T] >= 2) {
		if (team == CS_TEAM_CT) {
			...
			g_newteam[victim] = 1
			sic_announce_red(victim, "Mostantol TERRORISTA vagy!")
		}
	}
	...
	if (stat[CS_TEAM_T] - stat[CS_TEAM_CT] >= 2) {
		if (team == CS_TEAM_T) {
			...
			sic_announce_blue(victim, "Mostantol CT vagy!")
		}
	}
...
{
	if (g_newteam[player]) {
		if (g_newteam[player] == 1) {
			sic_announce_red(player, "Mostantol TERRORISTA vagy!")
		}
		if (g_newteam[player] == 2) {
			sic_announce_blue(player, "Mostantol CT vagy!")
		}
	}
